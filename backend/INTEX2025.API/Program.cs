...
builder.Services.ConfigureApplicationCookie(options =>
{
    ...
    options.Cookie.SameSite = SameSiteMode.None; // Allow the cookie to be sent in cross-site requests
    options.Cookie.Name = ".AspNetCore.Identity.Application"; // Custom cookie name
    options.LoginPath = "/login"; // Redirect path for unauthenticated requests
    options.Cookie.SecurePolicy = CookieSecurePolicy.Always; // Always send cookie over HTTPS
    options.Cookie.IsEssential = true; // Mark cookie as essential for the app functionality
...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowReactApp",
        policy =>
        {
            policy.WithOrigins("https://localhost:3030", "http://localhost:3030", "") // Allow specific origins
                .AllowCredentials() // Allows cookies to be included in cross-site requests
