...
// Cleans titles that have unclean data in titles
function sanitizeTitle(title: string): string {
  return title.replace(/[-?#()'":’‘“”.!&]/g, '');
}

// Loading symbol when movies rendering
const Spinner = () => (
...
// Tracks the showId being used for recs
interface TopRecProps {
  showId: number;
}

// Takes showId for recomendations
const GetTopRec: React.FC<TopRecProps> = ({ showId }) => {
  ...
  // Tracks loading status of images/titles
  if (loading) {
    return (
    ...
    return (
      // Displaying the information that can help them search
      <div
        className="genre-rec"
  ...
  return (
    <div className="genre-rec">
      ...
      {/* Navigation horizontal between movies */}
      <Slider {...sliderSettings}>
        {movies.map((movie, index) => (
          ...
          >
            {/* Displaying title and movie image */}
            <MoviePoster
              key={movie.show_id}
